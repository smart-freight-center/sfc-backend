...
import { ContractNegotiationState, ParticipantType } from 'entities';
import { TransferInitiationFailed, ContractNotFound } from 'utils/error';
...
export class EdcTransferService {
  ...
  private async getContractOffer(
    ...
    const assetFilter = {
      filterExpression: [builder.filter('asset:prop:id', assetId)],
    };
    ...
    return catalogs?.contractOffers[0];
  }
